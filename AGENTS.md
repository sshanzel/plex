...
packages/
  ...
  lang-python/   Python language plugin: WASM parser, extractor, resolver [M15]
...
plugin/                                     # the "plex" plugin — Claude AND Codex read this one dir
  ...
  commands/analyze.md                       # Claude: the /plex:analyze command (seed knowledge from PR history)
  agents/plex-reviewer.md                  # Claude: the reviewer subagent — REAL file, canonical
  agents/plex-analyzer.md                  # Claude: the analyzer subagent — REAL file, canonical
  scripts/gen-codex-skills.mjs             # generates codex/skills/ from the agents
  codex/skills/                            # GENERATED + committed (Codex has no agent/command type):
    plex-review/SKILL.md                   #   plex-reviewer → a plex-review skill (Codex-neutral tool wording)
    plex-analyze/SKILL.md                  #   plex-analyzer → a plex-analyze skill
...
- **The PR brain is a durable JSONL lineage log (ADR-46; was embedded Kùzu, ADR-30)** — rounds/findings/verdicts/comments as an append-only event log per target at `~/.plex/repos/<baseId>/lineage/<target>.jsonl` (base-keyed, so it survives `git worktree remove`), folded by `@plex/core` `foldLineage`; + the round-aware "changed-without-feedback" signal. **No FalkorDB, no Docker, no service, and no Kùzu for the brain** (a review opens Kùzu only for the code graph). A review needs nothing running. **Embeddings are optional** (ADR-30): without a provider the brain still records rounds/findings; only the semantic signals (unexplained changes + fix inference) are skipped. Set a provider once via `plex init` (→ `~/.plex/config.json`) or `PLEX_EMBEDDING_PROVIDER` + key. The brain is **per-machine** (keyed by local path); a *team-shared* brain is an explicit non-goal of the embedded design — options + the identity/concurrency tension are explored in [`docs/design/shared-brain.md`](docs/design/shared-brain.md).

### Native-integration gotchas (hard-won — see ADR-16, ADR-17)

- **FalkorDB is gone (M11/ADR-30); the brain is now durable JSONL, not Kùzu (ADR-46)** — review history (rounds/findings/verdicts/comments, the "lineage layer") is an append-only JSONL event log under the BASE repo's data dir, folded by `@plex/core` `foldLineage`. So a review opens Kùzu **only for the code-graph neighborhood (one open)** — the brain needs none. The **tsx Kùzu open-limit (ADR-17) still applies** to the code graph: keep tsx integration scenarios to **≤2 Kùzu opens**, and the full-review E2E stays a **node** check (`pnpm test:brain`); any auto-index/refresh runs in an **isolated child**.
...
7. **Languages plug in behind the `LanguagePlugin` seam** (`@plex/core` `lang.ts`): TS/JS via the TS compiler API (ADR-15), **Python via tree-sitter WASM (ADR-52, M15)** — never a second native addon beside Kùzu. Precise/`Refs` enrichment stays TS-only (Python emits `Imports` only); Python method symbols are dotted `Class.method` in BOTH the graph and deterministic layers (the ADR-47/48 key contract). Bump `GRAPH_VERSION` whenever extractor output changes in a way incremental can't reproduce.

## Analyzing PR review history (M4) — populate knowledge

**Distillation is LLM-only (ADR-20) and agent-only (ADR-51)** — no heuristic (a keyword matcher can't judge what's worth storing, so the LLM decides, and it can SKIP a cluster), and no standalone distiller (the connected coding agent IS the LLM; the old `plex analyze`/`analyze_history`/`PLEX_LLM_PROVIDER` path was removed). One path:
- **Agent-driven (rides your subscription, no API key):** the `/plex:analyze` command / `plex-analyzer` agent calls MCP `analyze_scan` (incremental — skips scanned PRs; `@plex/distill` fetches via `gh`, denoises, clusters) → the connected agent distills each cluster → `add_pitfalls` stores them → `consolidate_knowledge`.
...
- **Done:** M0 scaffolding, M1 review loop, M2 precision/determinism, M3 knowledge, **M4 review-history analysis**, M5 promotion + viz + build, **M6 PR brain**, **M7 incremental indexing + staleness + hooks**, **M8 incremental co-change + semantic waivers**, **M9 autonomous review**, **M10 one-command setup + centralized storage**, **M11 brain on Kùzu (FalkorDB removed; embeddings optional; auto-index)**, **M12 `awareness` findings + `acknowledge` verdict**. Post-M12 (ADR-only): worktree graph seeding (ADR-32), Kùzu Explorer / all Docker removed (ADR-33), PR auto-comment (ADR-34), chronological analysis (`--oldest`) + the clusterThreshold-for-real-embeddings fix, supply-chain hardening posture (ADR-35), **git-hook auto-index removed — the review is the trigger (ADR-36)**, **`plex.md` + markdown seeding + the promotion/external-runner surface removed; knowledge is learned only (ADR-37)**, **review-history analysis named `analyze` (product) + `@plex/distill` (technique) (ADR-38)**, **learned suppression — negative pitfalls on a Wilson basis: dismissals weighted (never a one-click kill), `demoted` triage tier, language-gated cross-repo promotion; positive confidence unified on Wilson too (ADR-39, `docs/design/negative-knowledge.md`)**, **worktree read-only graph sharing reverted to copy + stored in-workspace — Kùzu 0.11.3's read-only open SIGSEGVs on Linux (ADR-40)**, **suppression recency-decay (reject fades / waive persists, wall-time, decayed-Wilson → also the re-surface mechanism) + first-principles suppression via semantic title-embedding keys; re-surface probe dropped (ADR-41)**, **positive-pitfall decay — recency-weighted reinforcement + retrieval recency-tilt + pruning (provenance survives); `config.decay`, `Pitfall.lastReinforcedAt` (ADR-42, `docs/design/knowledge-decay.md`)**, **the background maintenance worker — a detached, debounced, idempotent sweep that maintains `main` (resolved from any worktree): closes landed loops → global KB, refreshes main's graph (isolated child), runs the ADR-42 consolidate/decay + incremental analyze; supersedes the flaky pr-responder bookkeeping AND the git hooks ADR-36 removed; `plex sweep`, auto-spawned by review/reconcile (ADR-43, `docs/design/maintenance-worker.md`)**, **observed outcome signals + unified Wilson confidence — `outcomeFor` confirms only on an OBSERVED code change (GitHub-outdated + merged) and abstains otherwise (killing the "merged ⇒ accepted" manufactured confirm); `confidenceFromOutcomes` is one Wilson definition across distill/`add_pitfalls`/consolidate; retrieval gains a confidence tilt; dead `outcomeWeight` deleted (ADR-44, `docs/design/outcome-signals.md`)**, **location-aware code-path memory — the Incident is an edge anchored to a `file#name` symbol; a pure `matchCodePath` overlay surfaces a lesson because you're touching the code it concerns, and a prior fix at a touched symbol is a regression sentinel (ADR-47, `docs/design/code-path-memory.md`)**, **location-scoped suppression (the negative twin) — a dismissal anchors to its symbol so suppression scopes to that instance instead of going repo-wide (dismiss one `console.log`, the rule still surfaces elsewhere); deterministic findings carry their enclosing symbol; `acknowledge`/`waive` tightened via `Waiver.symbol`; fail-open to repo-wide for legacy/symbol-less/explicit-scope dismissals (ADR-48, `docs/design/negative-knowledge.md`)**, **M14 surfacing the right lessons — a decay-immune `recurrence` retrieval re-rank tilt (`max(tiltFloor, n/(n+1))`, `n` = analyzed-incident count, positive-only, EXCLUDED from the `minScore` gate so a recurring lesson outranks a one-off without erasing it; the present-fix for a cold-started KB where every confidence is 0); distinct from `Finding.prevalence` (ADR-49)**, **reply-agreement as a graded weak confirm + an outcome backfill — `outcomeFor` gains a `corroborated` rung (PR-author reply-agreement on a merged PR, REST-only) weighted fractionally (`CORROBORATED_WEIGHT` 0.5; strong confirms stay 1.0, not the deleted bonus table); `refresh_outcomes` (MCP/CLI) + `replaceIncidents` re-derive analyzed outcomes from GitHub and consolidate — monotonic-upgrade-only, spares live accepts, safe no-op when unreachable; payoff is prospective (age-decay caps old evidence) (ADR-50, `docs/design/outcome-signals.md`)**, **distillation is agent-only — the standalone LLM distiller (`@plex/distill` LLM half, `analyzeRepo`, MCP `analyze_history`, `core` `LlmConfig`/`CompletionProvider`, `PLEX_LLM_PROVIDER`) is removed; the new `/plex:analyze` command + `plex-analyzer` agent run the `analyze_scan` → distill → `add_pitfalls` loop on your subscription; the maintenance worker drops its Analyze job (now 3 jobs) and the CLI slims to `init`/`index`/`serve`/`sweep` (ADR-51)**, **M15 Python support — the `LanguagePlugin` seam ADR-15 promised (`@plex/core` `lang.ts`, per-consumer capability maps) + `@plex/lang-python` (web-tree-sitter WASM; no native build) : dotted `Class.method` symbols, `__init__.py` barrels riding the existing transparency, fileSet-pure dotted/relative/src-layout import resolution (Imports-only; Refs stays TS), 7 Python deterministic rules with per-language prevalence, `Meta.graphVersion` forcing the one-time post-upgrade full re-index (ADR-52)**, **renames are followed, not dropped — co-change folds a renamed file's pre-rename commits onto its current path (`git log --name-status -M` + a newest→oldest temporal-alias `foldRenames`), the incremental path migrates the old node's `CoChange` edges onto the new node (from the captured `deletedEdges.co`), and code-path-memory Incidents + symbol-scoped Waivers re-anchor on rename from the diff's `oldPath`; `GRAPH_VERSION` 2→3 forces the one-time historical fold (ADR-53)**.
- **Out of scope (by request):** the multi-repo workspace in M5.

## Status

All milestones complete (M0–M15). See `docs/milestones/` for per-milestone records and `docs/adr/README.md` for the decision log (through ADR-53). **Languages: TS/JS + Python (ADR-52, M15).** **Renames are followed across co-change history + code-path/waiver anchors (ADR-53).** **Embedded — no Docker, no external/native services (ADR-30/33), no git hooks (ADR-36); review history is durable base-keyed JSONL, not Kùzu (ADR-46).** **One optional local node daemon** for data visibility: `plex serve` (M13/ADR-45, `@plex/viz-server`) serves an interactive Cytoscape UI at `http://127.0.0.1:2288` over the code graph + PR brain + knowledge — **on-demand by default** (`plex serve` / `npx … plex serve`; it's a viewer, not a capturer), opt into always-on with `ui.autoStart`/`PLEX_UI_AUTOSTART` (then the MCP spawns it on startup), opens Kùzu per-request so it never blocks a review, binds loopback only. `pnpm test` green (PR brain + worktree-seeding verified E2E under node via `pnpm test:brain` / `pnpm test:worktree`); `pnpm build` produces node-runnable binaries. Install: `plex init` (optional — asks for an embedding key + offers to index; the MCP comes from the plugin, so init does not register one) or just review — the **first review auto-indexes** the repo, and reviews auto-refresh the graph on drift (**the review is the only trigger — no git hooks to install; ADR-36**). Reviews are **autonomous** — submit findings and stop; a finding addressed by a later change is auto-`accept`ed (ADR-28) — this runs on the **next review itself**, locality-matched with **no embeddings or responder required** (ADR-36), so a standalone install still closes the accept-loop on re-review; reject stays agent/responder-driven, silence infers nothing. Verdicts also include **`acknowledge`** (M12) for a `note` finding confirmed intentional. **PR auto-comment** (ADR-34, opt-in `autoComment`) posts the ranked review back to the GitHub PR — one review, inline + summary, deduped per round — for `/pr-master:respond` to triage. Waivers suppress the same issue across rounds by meaning (semantic). `reconcile` (MCP `reconcile_outcomes` / `plex reconcile`) is the cheap "did the author fix these?" check for after a push — it matches a fix to a finding by semantic title OR file/line **locality** (so a restructuring fix still reconciles), and returns a `reason` so `accepted: 0` is never a black box. The MCP server exposes 14 tools; per-repo data lives outside the repo (`~/.plex/repos/<id>`). A linked git worktree keeps only its **disposable code-graph copy** in-workspace at `<worktree>/.plex` (self-gitignored, dies with the worktree, ADR-40); its **review history + verdicts are base-keyed and durable** under the base repo's `~/.plex/repos/<baseId>/` (ADR-46), so they survive `git worktree remove`.

**The review target is BASE-keyed (ADR-46).** All lineage writes (round recording, finding writes, verdicts, reconcile, `record_outcome`) key off **`reviewTargetFor(repoPath, src)`** = `reviewTarget(basename(baseRepoPath(repoPath)), src)` — the BASE repo basename (the primary checkout a worktree branches from), never the worktree dir name or the code graph's `repo` meta. So a review from a worktree and one from the base of the SAME PR resolve to ONE target, stored durably under the base repo's data dir. Because the key is base-derived, the brain **cannot split across worktree names** — the old `Brain.healSplitTarget` guard + `HEAL_RELABEL_ORDER` were **deleted** (ADR-46); do not reintroduce them. Route every new lineage key through `reviewTargetFor`.
