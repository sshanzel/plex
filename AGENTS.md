...
.claude/
  ...
  skills/plex-review-responder   -> ../../.agents/skills/plex-review-responder   (symlink)
...
.agents/skills/                            # the REAL skill files (Codex/other agents read these)
  ...
  plex-review-responder/SKILL.md             # resolve PR feedback + close the Plex learning loop
...
2. Address feedback with the `plex-review-responder` skill → after pushing fixes it calls `reconcile_outcomes` (auto-`accept`s what you fixed) and `record_outcome reject`/`acknowledge` only for explicit dismissals. Silence is never a verdict.
