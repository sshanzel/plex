...
- **Done:** M0 scaffolding, M1 review loop, M2 precision/determinism, M3 knowledge, **M4 mining**, M5 promotion + viz + build, **M6 PR brain (round-aware review + changed-without-feedback + audit log)**, **M7 incremental indexing + graph staleness + git hooks**, **M8 truly-incremental co-change + semantic waiver suppression**.
- **Out of scope (by request):** the multi-repo workspace in M5.

## Status

All milestones complete (M0–M8). See `docs/milestones/` for per-milestone records and `docs/adr/README.md` for the 27 decisions. `pnpm test` green (48 unit + 10 integration); the PR brain is verified E2E under node via `pnpm test:brain`; `pnpm build` produces node-runnable binaries. Keep the graph fresh with `plex index --incremental` (TS *and* co-change are now incremental — or `plex install-hooks`); a review **auto-refreshes** the graph incrementally if it has drifted behind HEAD (opt out with `autoIndex:false`). Waivers suppress the same issue across rounds by meaning (semantic), surviving line drift / rewording. The MCP server exposes 13 tools (FalkorDB + an embedding provider required for the review flow — M6).
