...
| `src/collect.ts` | Read each store into a `GraphPayload`: `collectCode` (Kùzu code graph — **lands on a connected hub neighbourhood** for big repos: rank files by coupling degree, grow each hub + its neighbours up to `CODE_LANDING_CAP` = 80 so the landing shows real structure, not an 800-file wall or isolated dots; small repos show in full) / `searchFiles` (parameterized `$q` path-CONTAINS — reaches files OUTSIDE the landing set for the UI search) / `expandCodeFile` (expand also runs `linkSymbolIncidents`, ADR-47: a Symbol → the incidents anchored at it by `file#name` key (solid) or line-overlap (dashed/`inferred`), so clicking a symbol shows its concern history), `collectBrain` (durable JSONL lineage via `@plex/core` `foldLineage` — ADR-46, no Kùzu), `collectKnowledge` (JSON, optional repo-origin scope; incident nodes carry the `symbol`/`line` anchor), `collectLineage` + pure `linkLineage` (brain ⨝ knowledge). |
| `src/server.ts` | `startServer` — routes `/`, `/healthz`, `/api/repos`, `/api/graph/{code,brain,knowledge}`, `/api/expand`, `/api/search` (code-graph file search → `searchFiles`); binds 127.0.0.1; maps `RepoBusyError` → 503. |
...
| `src/ui.ts` | `renderAppHtml` — the whole single-page Cytoscape app (CDN + SRI-pinned 3.30.2, same hash as the M5 static viz). **Legibility encoding:** node border by `outcome` (green=resolved, red=dismissed), a thick orange ring + ⚠ label + panel banner for a **regression sentinel** (a resolved concern anchored to a symbol); edge labels shown on the relationship graphs (`graph != "code"`) so `became`/`from`/`raised in` read; the detail panel leads with a `summaryFor` one-liner; legend gains the outcome/sentinel key. **Knowledge clustering:** each Pitfall/Suppression is a Cytoscape **compound parent** (`:parent` tinted box, type-coloured) holding its Incidents (`VizNode.parent` set in `collectKnowledge`); `knowledgeGrid` lays the boxes in a deterministic grid and packs each box's incidents in a mini-grid (`preset` positions) — cose/force layouts band the edge-less clusters, so a hand-rolled grid is used instead of pulling in `fcose`. Incident dots drop their label (shown on select) since the box title + outcome ring carry the meaning. **Code-graph search** (`doSearch`/`fetchSearch`): instantly highlights loaded matches, then debounced-fetches `/api/search` to ADD matching files NOT in the landing set (the landing shows only hubs). **Empty state** (`showEmptyState`): a 0-node payload renders a centered note + graph-specific CTA (knowledge → `plex analyze`) instead of a black void. All client-side, still `textContent`-only (XSS-safe). |
