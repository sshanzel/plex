...
- **`runGit` retries a transient SPAWN failure** via the shared `retryTransientSpawn`/
  `isTransientSpawnError` policy in **`@plex/core`** (`EAGAIN`/`ENOMEM`/`EMFILE`/… where the child
  never forked, e.g. under CI fork-storm) but NOT a non-zero exit (`code` is a number — a real failure
  like a bad ref). The policy lives in core so `@plex/code-graph`'s `headSha` shares it (the audit
  found that copy un-retried); `isTransientSpawnError` is still re-exported from here for callers.
  This is a **correctness** fix, not flake
