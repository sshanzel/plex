...
describe('resolveConfig', () => {
  ...
  it('partial neighborhood / mining overrides preserve sibling fields', () => {
    const cfg = resolveConfig({ neighborhood: { maxHops: 4 } as never, mining: { minClusterSize: 3 } as never });
    expect(cfg.neighborhood.maxHops).toBe(4);
    expect(cfg.neighborhood.maxNeighbors).toBe(defaultConfig.neighborhood.maxNeighbors);
    expect(cfg.mining.minClusterSize).toBe(3);
    expect(cfg.mining.maxPrs).toBe(defaultConfig.mining.maxPrs);
  });
