...
1. **List PRs**: `gh pr list --state <merged|all> --limit maxPrs` (`config.analyze.maxPrs`, default 100).
...
3. **Provenance Incidents**: each substantive comment → `Incident` id `inc:analyzed:<commentId>`,
   `source: 'analyzed'`, `snippet: body.slice(0, 300)`, `outcome: outcomeFor(c)` — deduped against
   existing incident ids so re-runs never duplicate.
4. **Embed + cluster**: one batched `embed()` over all bodies, then
   `threshold = adaptiveCosineThreshold(vectors, { fallback: config.analyze.clusterThreshold })`:
...
Lives at `repoPaths(...).analyzeStateFile` = `<repo-data>/analyze-state.json` —
**`~/.plex/repos/<id>/analyze-state.json` by default** (in-repo `<repo>/.plex/analyze-state.json` only
with the `PLEX_DATA_DIR=.plex` opt-in). Shape: `{ repo, scannedPrs: number[], lastRun }`; owned by
`packages/engine/src/analyze.ts` (`loadAnalyzeState`/`saveAnalyzeState`), not this package — `scanHistory`
just receives `alreadyScanned` and returns the cumulative `scannedPrs`. Flags: `--reset` ignores the
saved cursor (re-scan everything; incident dedupe still prevents duplicates); `--all` analyzes unmerged
