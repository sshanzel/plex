...
3. **Staleness check + drift refresh (ADR-25/36)** — read the sidecar `head.sha` (no Kùzu open), and if HEAD is definitively ahead, refresh incrementally in the same isolated-child way. The review is the *only* refresh trigger (no git hooks, ADR-36). When the diff DELETES files, their direct dependents are captured from the **pre-refresh** graph first (`captureDeletedNeighbors` — one short extra open on this path only): the refresh DETACH-DELETEs a committed deletion's node, which would otherwise erase its blast radius (a dogfood catch).
