...
- `src/guards.ts` — pure decision helpers extracted so silent-failure guards are unit-testable without opening Kùzu: `HEAL_RELABEL_ORDER` (Round-last crash-safe re-key for `healSplitTarget`), `shouldRecordRound` (never persist a round with an empty headSha), `projectableOutcome`/`OUTCOME_BY_KIND` (skip a brain outcome projection for an empty findingId — a no-op MATCH that re-opens the finding). Imported by `brain.ts`/`review.ts`/`knowledge.ts`.
