...
## ADR-32 — Worktrees seed their code graph from the base (main) worktree ✅ (built)
...
**Decision.** When indexing a path that has no graph yet AND is a *secondary* worktree whose **main** worktree (first `git worktree list`) is already indexed: **copy** the base graph dir, then run an **incremental update** that applies only this branch's diff (`mainIndexedSha..branchHEAD`). The base is read-only — the copy is independent — so N worktrees can never affect it. Falls back to a full build if the copy/incremental fails (e.g. the base was mid-write).
**Consequences.** A new worktree is **seconds, not a full parse** (copy a ~tens-of-MB dir + extract only the changed files); reviews/auto-index benefit automatically (the auto-index spawns `plex index`, which seeds). One durable base graph, cheaply cloned per branch, never mutated by any worktree — exactly the intended model. *Caveat:* copying a Kùzu dir while the base is being written could yield an inconsistent copy; the incremental step then fails and we full-build — correct, just not seeded that time.
