...
**Consequences.** Pay the big parse cost once; keep fresh in O(changed files); the blast radius stops silently drifting and new files stop being invisible. Workspace/runtime couplings that static resolution misses remain covered by co-change (ADR-06). Incremental indexing is Kùzu-only (no FalkorDB), so it is safely covered by the tsx integration runner (ADR-16/17). *Deferred:* truly incremental co-change (store raw per-pair observations + last commit, append `<sha>..HEAD`).
