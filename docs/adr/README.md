...
## ADR-22 — FalkorDB is the per-PR working memory, not just viz 🧪 (M6)
**Context.** ADR-07 scoped FalkorDB to an *ephemeral visual mirror* of the structural blast radius. In practice the blast radius is computed entirely from **Kùzu + the diff**; FalkorDB only received a copy to look at. A graph you only look at "produces nothing" — it carries no weight in the review. We want FalkorDB to be a real cog while keeping ADR-07's "never hard-fail / optional" guarantee.
**Decision.** Promote the per-PR FalkorDB graph from *viz mirror* to the **working memory of a review target** — the materialized "brain of the PR." Beyond the blast radius it holds `Round`, `Finding`, `Verdict`, and `Comment` nodes with their edges, and the orchestrator **queries it** to derive round-aware signals (§ADR-23). **Durable truth still lives in JSON/Kùzu** (verdicts, incidents, per-round snapshots), so the graph is fully *rebuildable* and a FalkorDB flush loses nothing. When FalkorDB is disabled/unreachable the **same signals are computed in-process** from the durable snapshots — FalkorDB is no longer "just a picture," but it is still never load-bearing for correctness.
**Consequences.** FalkorDB earns its place via cross-round / cross-blast-radius traversal queries that are awkward in flat JSON, and stays inspectable live. Cost: a durable substrate plus its graph projection (acceptable — the substrate is small and the projection is derived). Supersedes the "ephemeral viz only" framing of ADR-07 while preserving its optionality.

## ADR-23 — Round-aware review; "changed-without-feedback" 🧪 (M6)
...
**Decision.** A review **target has rounds**: a round = a review invocation at a distinct head SHA. Each round persists a compact snapshot (`headSha`, changed file ranges, finding/verdict/comment ids) under `<repo>/.plex/rounds/<target>/`. On a new round the orchestrator diffs round *N* vs *N−1* and classifies each changed range as **feedback-driven** (overlaps a prior finding or PR-thread comment) or **unexplained** (changed with no driving comment/finding) — surfacing the *unexplained* set as a high-priority note to the fresh reviewer ("these lines moved with nothing explaining why — scrutinize"). PR-thread comments are ingested per round (reusing the mining fetcher) as **facts**.
