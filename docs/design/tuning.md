...
| `CORROBORATED_WEIGHT` | `0.5` | **one named constant (ADR-50)** | the fractional weight of a `corroborated` confirm (a PR-author reply-agreement — a *claimed* fix) in the Wilson confirm count, vs `1.0` for an observed `fixed`/`accepted`/`reverted`. NOT a revival of the deleted `outcomeWeight` *bonus* table (strong confirms stay 1.0; this only discounts a weaker *source*); Wilson takes the fractional count unchanged. A single constant in `stats.ts` (overridable param on `confidenceFromOutcomes`), graduates to config if tuning demands. |
...
| positive-pitfall decay | `halfLifeDays` **365** / `retrievalTiltFloor` **0.5** / `pruneFloor` **0.1** / `pruneMinAgeDays` **365** | **the ADR-42 knobs** | `config.decay` — settable via `~/.plex/config.json` (`decay` block, partial-merges) or `PLEX_DECAY_*` (`config-load.ts`). `consolidatePitfalls` recency-weights each incident's confirm/refute by `recencyWeight = 0.5^(ageDays/halfLifeDays)` before the unchanged Wilson bound (positives age like the long suppression end — `halfLifeDays` mirrors `waive`/co-change 365); `rankAndSlim` tilts the retrieval score by `max(retrievalTiltFloor, recencyWeight(lastReinforcedAt age))` (read-only — never touches confidence; floor keeps an old-but-real lesson visible); pruning drops a pitfall that's been **refuted** (`refutes > 0`) AND fell below `pruneFloor` decayed confidence AND quiet > `pruneMinAgeDays` (non-repo-scoped, has-incidents — provenance survives; the `refutes > 0` gate stops a thin never-refuted lesson from being deleted just for aging). Undated incidents → weight 1 (no decay). Runs via the ADR-43 worker. `promotion.ts` / `retrieve.ts`. |
| recurrence re-rank tilt | `max(retrievalTiltFloor, n/(n+1))` | **principled shape (ADR-49)** | `recurrenceWeight(p)` in `rankAndSlim` — a third retrieval tilt, `n` = the count of analyzed-provenance incidents (`inc:analyzed:*`, NOT raw `incidentIds.length` — consolidate unions live accepts in, which would inflate it; positive-polarity only). Monotonic + saturating, reuses `retrievalTiltFloor` (0.5). **Re-rank, NOT a gate**: it multiplies the sort/returned score but is **excluded from the `minScore` cut** (gate = `cosine × recency × confidence`), so a recurring lesson outranks a one-off without erasing a valid one-off (gating on it regressed recall — every `n=1` sits at the floor → uniform discount → borderline hits dropped). The ONLY axis that does not decay with age — the lever for a cold-started historical KB. Distinct from `Finding.prevalence` (repo-commonness, demotes). `retrieve.ts`. |
