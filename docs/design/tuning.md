...
| suppression decay half-life | reject **30d** / waive **365d** | **the one accepted knob (ADR-41)** | `config.suppression` — each dismissal's weight halves every N days by verb (`recencyWeight` = `0.5^(ageDays/hl)`), feeding decayed fractional counts into `suppressionTier`. A `reject` ("not now") fades ~12× faster than a `waive` ("this is wrong"); corrections are durable (no knob). Wall-time clock (incidents carry `ts`); review-count is the documented alternative. This is also the re-surface mechanism (an aged suppression slides back out). `stats.ts`. |
