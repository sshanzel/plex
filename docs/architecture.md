...
- a finding **addressed by a later change** → auto-`accept` on the next review (or via `reconcile`); an **explicit dismissal** → `reject` (responder); a `note` finding confirmed intentional → **`acknowledge`** (M12, no down-weight); **silence** → nothing.
