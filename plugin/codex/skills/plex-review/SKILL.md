...
3. **Reason from first principles** — as if reviewing this code for the FIRST time.
   ...
   **Severity calibration** — severity and confidence are independent axes. Set both honestly:
   ...
   - `awareness` (displays as **Note**) — intentional-looking but worth confirming ("is this threshold right for production load?", "is this event emit intentionally duplicated?")
   ...
   - Use **`awareness`** (NOT `nit`) for something *worth surfacing for confirmation* that isn't a defect: a duplicated event-emit across two surfaces, a non-obvious-but-deliberate-looking pattern, a "is this intentional?" — where raising it IS the value even if the answer is "yes, intentional." These display as **Note** in the table. Plex surfaces them in their own triage bucket and won't auto-suppress them.
